...
export class avatar extends component {
	...
	async refresh () {
		await globalThis.fetcher(`/api/my/avatar/`, {
			method: "get"
		}, {
			200: async _response => {
	...
	async generate () {
		const firstName = globalThis.session.identity.firstName.substring(0, 1);
		const lastName = globalThis.session.identity.lastName.substring(0, 1);
		...
		context.fillText(text, (canvas.width / 2) - 1, (canvas.height / 2) + 2);
