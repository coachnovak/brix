...
export default async (_app, _options) => {
	_app.get("/:room", {
		...
		const roomResult = await _app.mongo.db.collection("rooms").findOne({ _id: new _app.mongo.ObjectId(_request.params.room) });
		...
		const invites = await _app.mongo.db.collection("invites").find({ "room._id": new _app.mongo.ObjectId(_request.params.room) }).toArray();
	...
	_app.post("/:room", {
		...
		const senderResult = await _app.mongo.db.collection("users").findOne({ _id: new _app.mongo.ObjectId(_request.user.user) });
		if (!senderResult) return _response.status(404).send({ message: "You couldn't be found." });

		const recipientResult = await _app.mongo.db.collection("users").findOne({ email: _request.body.email });
		...
		const invitedResult = await _app.mongo.db.collection("invites").findOne({ "room._id": roomResult._id, "sender._id": senderResult._id, "recipient._id": recipientResult._id });
	...
	_app.delete("/:id", {
		...
		const invitedResult = await _app.mongo.db.collection("invites").findOne({ _id: new _app.mongo.ObjectId(_request.params.id) });
		if (!invitedResult) return _response.status(404).send({ message: "We couldn't find the invitation." });

		const roomResult = await _app.mongo.db.collection("rooms").findOne({ _id: invitedResult.room._id });
		if (!roomResult) return _response.status(404).send({ message: "Room couldn't be found." });
		...
		const response = await _app.mongo.db.collection("invites").deleteOne({ _id: new _app.mongo.ObjectId(_request.params.id) });
